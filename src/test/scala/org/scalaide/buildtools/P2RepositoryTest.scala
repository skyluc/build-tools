...
"""
  ...
  @Test def repoWithValidUrl {
    val goodUrl = this.getClass().getClassLoader().getResource("site_1")
    val goodUrlRepo = P2Repository.fromUrl(goodUrl)
    Assert.assertTrue("Should be a valid P2Repository", goodUrlRepo.isInstanceOf[ValidP2Repository])
  }
}